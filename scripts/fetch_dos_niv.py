#!/usr/bin/env python3
"""
Download DOS Monthly Nonimmigrant Visa Issuances by Nationality and Visa Class
...
PAGE_URL = (
    ...
    "visa-statistics/nonimmigrant-visa-statistics/"
    "monthly-nonimmigrant-visa-issuances.html"
)

OUTPUT_DIR = Path("data/raw/dos_niv")
START = (2022, 10)
END = (2025, 8)
...
def normalize_text(text: str) -> str:
    # Replace en/em dashes with standard hyphen-like spacing handling
...
def parse_month_year_from_title(title: str) -> Tuple[int, int, str]:
    """
    Example title:
      'October 2022 - NIV Issuances by Nationality and Visa Class'
    """
    m = re.match(
        r"^([A-Za-z]+)\s+(\d{4})\s+-\s+NIV Issuances by Nationality and Visa Class$",
        title,
    )
    if not m:
...
def scrape_excel_links(session: requests.Session) -> Dict[Tuple[int, int], MonthlyFile]:
    ...
    for li in soup.find_all("li"):
        ...
        for a in anchors:
            ...
            if "NIV Issuances by Nationality and Visa Class" in label:
                title_anchor = a
        ...
        # Keep only the nationality/class report, not the "Post and Visa Class" report.
        if "NIV Issuances by Nationality and Visa Class" not in title:
            continue
...
def main() -> None:
    ...
    with requests.Session() as session:
        ...
        for year, month in expected:
            ...
            filename = f"{year}-{month:02d}_dos_niv_nationality_visa_class.xlsx"
